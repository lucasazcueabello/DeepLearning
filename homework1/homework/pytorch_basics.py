...
class PyTorchBasics:
    ...
    def make_it_pytorch_4(x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        ...
        """
        return torch.where(y > torch.mean(x), torch.ones(y.shape), 0.).sum()
    ...
    def make_it_pytorch_extra_2(x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        ...
        """
        return torch.any((torch.abs(x.unsqueeze(1) - y.unsqueeze(0)) < 1e-3), dim=1).sum()