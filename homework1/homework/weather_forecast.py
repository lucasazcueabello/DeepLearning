...
class WeatherForecast:
    ...
    def find_the_most_extreme_day(self) -> torch.Tensor:
        ...
        """
        return self.data[torch.arange(self.data.size(0)), torch.argmax(torch.abs(self.data - torch.mean(self.data, dim=-1, keepdim=True)), dim=-1)]
    ...
    def max_last_k_days(self, k: int) -> torch.Tensor:
        ...
        Returns:
            tensor of size (k,)
        """
        return torch.max(self.data[self.data.size(0)-k:], dim=-1).values
    ...
    def predict_temperature(self, k: int) -> torch.Tensor:
        ...
        """
        return torch.mean(self.data[self.data.size(0)-k:][:].flatten())
